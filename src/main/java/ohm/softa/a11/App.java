...
public class App {
	...
	private static void printCanteens() throws ExecutionException, InterruptedException {
		...
		openMensaAPI.getCanteens()
			.thenApply(response -> {
				...
				CompletableFuture<List<Canteen>> combinedFuture = null;

				for(int i = 2; i <= pageInfo.getTotalCountOfPages(); i++) {
					if (combinedFuture == null) {
						combinedFuture = openMensaAPI.getCanteens(i);
					} else {
						CompletableFuture<List<Canteen>> future = openMensaAPI.getCanteens(i);
						combinedFuture = combinedFuture.thenCombine(future, ListUtil::mergeLists);
					}
				}
				try {
					canteens.addAll(combinedFuture.get());
	...
	private static void printMeals() throws ExecutionException, InterruptedException {
		...
		if(currentCanteenId <= 0) {
			System.out.println("You have to select a canteen first!");
			return;
		}

		final String dateString = dateFormat.format(currentDate.getTime());

		//check state of the canteen
